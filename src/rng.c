...
bool virtio_rng_driver(volatile EcamHeader* ecam) {
    ...
    if (ecam->status_reg & PCI_STATUS_REG_CAPABILITIES) {
        ...
        while (true) {
            ...
            switch (cap->id) {
                case 0x09:
                    if (!virtio_rng_setup_capability(ecam, (VirtioPciCapability*) cap)) {
                        printf("virtio_rng_driver: failed to setup virtio capability at 0x%08x\n", (u64) cap);
                        return false;
                    }
                    break;
...
bool virtio_rng_setup_capability(volatile EcamHeader* ecam, volatile VirtioPciCapability* cap) {
    switch (cap->cfg_type) {
        case VIRTIO_PCI_CAP_CFG_TYPE_COMMON:
            if (!virtio_rng_setup_cap_cfg_common(ecam, cap)) {
                printf("virtio_rng_setup_capability: failed to setup common configuration\n");
                return false;
            }
            break;
        ...
        case VIRTIO_PCI_CAP_CFG_TYPE_NOTIFY:
            if (!virtio_rng_setup_cap_cfg_notify(ecam, cap)) {
                printf("virtio_rng_setup_capability: failed to setup notify configuration\n");
                return false;
            }
            break;

        case VIRTIO_PCI_CAP_CFG_TYPE_ISR:
            if (!virtio_rng_setup_cap_cfg_isr(ecam, cap)) {
                printf("virtio_rng_setup_capability: failed to setup isr configuration\n");
                return false;
            }
            break;
        ...
        case VIRTIO_PCI_CAP_CFG_TYPE_COMMON_ALT:
            printf("virtio_rng_setup_capability: ignoring configuration type: %d\n", cap->cfg_type);
            break;

        default:
            printf("virtio_rng_setup_capability: unsupported configuration type: %d\n", cap->cfg_type);
            return false;
    }
...
bool virtio_rng_setup_cap_cfg_common(volatile EcamHeader* ecam, volatile VirtioPciCapability* cap) {
    ...
    if (queue_size != VIRTIO_OUR_PREFERRED_QUEUE_SIZE) {
        printf("virtio_rng_setup_cap_cfg_common: device renegotiated queue size from %d to %d\n", VIRTIO_OUR_PREFERRED_QUEUE_SIZE, queue_size);
    }

    cfg->device_status |= VIRTIO_DEVICE_STATUS_FEATURES_OK;
    if (!(cfg->device_status | VIRTIO_DEVICE_STATUS_FEATURES_OK)) {
        printf("virtio_rng_setup_cap_cfg_common: device rejected feature ok\n");
        return false;
    }
    ...
    if (!(cfg->device_status | VIRTIO_DEVICE_STATUS_DRIVER_OK)) {
        printf("virtio_rng_setup_cap_cfg_common: device rejected driver ok\n");
        return false;
    }
